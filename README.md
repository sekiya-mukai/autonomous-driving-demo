その通りです。前の回答では、こちらの表示用コードブロックと**実際にREADMEへ貼り付けるMarkdown**を混同していました。申し訳ありません。

 今回は、**外側のwriting block自体を使わず**、`README.md` の内容を**1つのMarkdownコードブロックの中に完全に入れます**。\
 そのため、コピーすると `## 概要` も ```` ```mermaid ```` も含めて、そのまま `README.md` に貼れます。
...
これなら**一番外側のコードブロックを丸ごとコピー**すれば、`README.md` として必要な
